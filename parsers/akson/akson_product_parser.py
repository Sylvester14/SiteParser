...
class AksonProductParser(ProductParser):        
    ...
    def _parse_price(self) -> int:
        price_element = self.soup.find("div", class_="title-m-semibold text-headline")
        
        if(price_element is None):
            raise Exception(f"Error ProductParser._parse_price(): price is None!. Product url: {self.url}")
    ...
    def _parse_name(self) -> str:
        name_element = self.soup.find("h1", class_="name title-s-bold text-headline")
        
        if(name_element is None):
            raise Exception(f"Error ProductParser._parse_name(): name is None!. Product url: {self.url}")
    ...
    def _parse_features(self) -> dict: 
        try:
            result = {}
            features_title = self.soup.find("h2", string="Характеристики")
            ...
            raise Exception(f"Error ProductParser._parse_features(): features parsing error!. Message: {e}. Product url: {self.url}")
